...
function assertionE2E(elementType, element, assertion, value, endPoint, _self) {
    ...
    const expectFn = {
        'have.text': (element, text) => {
            element.should('be.visible').then(($el) => {
                let extractedText;

                // Si el elemento tiene múltiples <span>, extraer y unir los textos
                if ($el.find('span').length > 0) {
                    extractedText = [...$el.find('span')].map(span => span.innerText).join(' ');
                } else {
        ...
        'have.attr': (element, value) => {
            if (!attribute){
                    throw new error (`'have.attr' requires an attribute name (e.g. 'have.attr:href'), but none was provided.`);
            }
            return element.should('have.attr', attribute, value);
        }
    };
    if(!expectFn[assertion]){
        throw new Error(`Assertion '${assertion}' not supported.`);
    }
