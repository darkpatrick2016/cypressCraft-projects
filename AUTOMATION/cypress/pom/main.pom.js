...
module.exports = class Main {
    constructor(elements=null) {
        this.helper          = require('../settings/helpers.js');
    ...
    _getMail(){
        cy.task('createMail', null , {log: false}).then(mail => {
            expect(mail).contain('@yopmail.com');
            this.email = mail;

            cy.writeFile(this.constants.PATH_EMAIL, mail); // guardamos todos los correos generados
        })
    }
    ...
    _validate(elementType, elementId, condition, content=null){
        const endPoint = this.currentEndPoint;
         ...
         // Detectamos si el content es un alias conocido
         if (content === 'generatedFirstName') {
           cy.get('@generatedFirstName').then((resolvedValue) => {
             this._validate(elementType, elementId, condition, resolvedValue);
           });
           return;
         }
       
         // Verificación personalizada para tabla
         if (elementId === 'tablaDeRegistros' && condition === 'tiene el texto') {
           const expectedValue = content
             ? (isDynamic(content) ? extractAndSetDynamicValue(content, endPoint, this) : content)
             : null;
       
           cy.get('.rt-table .rt-tbody')
             .contains('div.rt-td', expectedValue)
             .should('be.visible');
    ...
    _waitFormSeconds(seconds) {
        cy.wait(seconds * 1000);
    }

    originalUrl = null;
   _handleLink(elementId) {
    const element = this._getElement(elementId);
    ...
    element.then($el => {
        ...
        cy.url().then(currentUrl => {
            if (href && href !== currentUrl && !href.startsWith('javascript')) {
                // Link abre otra página
                console.log(`Abriendo link "${elementId}" con href: ${href}`);
                
                if (!this.originalUrl) this.originalUrl = currentUrl;
                
                cy.visit(href).then(() => {
                    // Volver automáticamente a la original
                    cy.visit(this.originalUrl);
                });
            } else {
                // Link ejecuta JavaScript o no cambia la página
                console.log(`Click en link "${elementId}" sin cambio de página`);
                element.click();

                // Detectar mensaje generado
                cy.get('body').then($body => {
                    if ($body.text().includes('Link has responded')) {
                        console.log('Mensaje detectado:', $body.text());
                        }
                    });
                }
            });
